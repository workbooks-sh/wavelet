...
You are the creative director. Use the wavelet CLI (already on your PATH)
...
- `eases.css` — copy/paste the extended ease table from
  `packages/workbooks/skills/wavelet-director/eases.css` so each scene can
  `@import` it (or paste the `:root` block inline per scene; either works)
- `index.html` — the multi-scene manifest
- `commercial.mp4` — the final render via `wavelet render index.html -o commercial.mp4`
...
wavelet workflow run commercial --workdir . --text

# Paid spend stayed under ceiling
jq -s 'map(.cost_estimate_usd // 0) | add' trace.wavelet.jsonl  # or wherever trace lives
```

## Where to learn

The canonical recipe is at
`packages/workbooks/skills/wavelet-director/SKILL.md` (relative to repo
root). The ease table is at
`packages/workbooks/skills/wavelet-director/eases.css`.

`wavelet --help` lists every subcommand. Each has its own `--help`.

`wavelet pipelines show commercial` prints the eight-stage spec.

## Success criteria

- `commercial.mp4` exists, is non-trivial size (~1 MB+), and plays.
- `wavelet workflow run commercial --workdir .` reports every stage as `complete`.
- All seven "required palette use" items above are visible in the
  ...
  and inspecting).
- Total paid spend ≤ $5.00 USD per `trace.wavelet.jsonl`.
