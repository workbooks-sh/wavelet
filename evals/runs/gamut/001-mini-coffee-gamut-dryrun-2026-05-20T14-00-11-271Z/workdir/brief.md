...
You are a creative director. Use the gamut CLI (already on your PATH)
...
- Use **dry-run** mode on every `gamut shot txt2vid|img2vid`, `gamut image scene-still`,
  and `gamut music gen` for this sanity test. The point is exercising the
  pipeline shape, not paying for one shot.
- For the final render (`gamut render`), live calls are fine — render is
...
3. `screenplay.json` — `gamut screenplay parse` output
4. `velocity.json` — `gamut velocity propose` output
5. `storyboard.json` — `gamut storyboard plan` output
...
`vendor/workbooks/skills/gamut-director/SKILL.md` (relative to the repo
...
`gamut --help` lists every subcommand. Every subcommand has its own
`--help`.

## Success criteria

- `commercial.mp4` exists, is non-zero size, and plays.
- `gamut workflow run commercial --workdir .` (where `commercial` is
  the pipeline name at `packages/gamut/pipeline_defs/commercial.yaml`)
