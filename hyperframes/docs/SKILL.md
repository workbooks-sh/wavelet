---
name: hyperframes
description: Authoring guide for HyperFrames scenes inside a wavelet composition. Each <gm-scene> is a HyperFrames scene — full HTML/CSS/JS authoring power, animation written by hand, no recipe library. Read this before writing any scene.
---

# HyperFrames — authoring scenes for wavelet

A HyperFrames scene is the visual content of a single `<gm-scene>` inside a wavelet composition. You write HTML, CSS, and JS — including any animation library you want (GSAP, anime.js, CSS Web Animations, raw WebGL). The wavelet runtime gives you a fixed 1920×1080 canvas, fires `hf:ready` when your scene mounts, fires `hf:tick` on every frame, and tears the scene down when its time ends. Everything else is your call.

There are no recipes here. No `intent="reveal"`. No `kind="fade"`. The wavelet runtime renders exactly what you write. Missing motion is missing motion — the runtime doesn't paper over it with a "sensible default."

## Approach

Before writing any scene, think at a high level:

1. **What** — what should the viewer experience? Identify the narrative arc, key moments, and emotional beats.
2. **Structure** — how many scenes, which are inline vs external (`<gm-include src=>`), what tracks carry what (video, audio, overlays, captions, grades).
3. **Timing** — which clips drive the duration, where do scene overlaps land (overlaps = "transitions" — there's no `<transition>` element), what's the pacing.
...
<HARD-GATE>
Before writing ANY scene HTML, you MUST have a visual identity defined. Do NOT write scenes with default or generic colors.
...
### External images — DO NOT guess Wikimedia URLs

Wikimedia thumbnail URLs contain an MD5 hash of the canonical filename
(`/wikipedia/commons/thumb/<hash1>/<hash2>/<file>/<size>px-<file>.png`).
You **cannot** predict the hash. URLs you "recall" from training data
are almost always wrong and 400 / 404 at runtime.

**Use the hash-free `Special:FilePath` redirect instead:**

```
https://commons.wikimedia.org/wiki/Special:FilePath/<Filename.ext>?width=<n>
```

Example — Seal of San Francisco at 1024px wide:

```html
<img src="https://commons.wikimedia.org/wiki/Special:FilePath/Seal_of_San_Francisco.svg?width=1024"
     crossorigin="anonymous"
     alt="Seal of San Francisco">
```

Wikimedia 302 → 301 → 200 to the actual thumbnail; the browser follows
transparently. The filename is exactly the title shown on the Commons
file page (spaces become underscores; case-sensitive). When in doubt,
check `https://commons.wikimedia.org/wiki/File:<Filename.ext>` first.

For non-Commons sources (random web URLs), prefer **uploading the asset
to the workbook** over hotlinking — the asset survives reloads, doesn't
depend on a third party staying up, and never trips a 4xx during render.
